...
function RoleChip({ role, lang }: { role: OrgRole; lang: Lang }) {
  ...
  if (role === "OWNER") {
    return <span className="text-[13px] text-[var(--text)]">{label}</span>;
  }
...
export function MembersTable({
  ...
  return (
    <Card>
      <CardHeader>
        <h2 className="font-semibold text-[var(--text)]">{t("team.members.title", lang)}</h2>
      </CardHeader>
                ...
                return (
                  <Fragment key={m.userId}>
                    <Tr>
                      ...
                      <Td>
                        <span className="text-[var(--text-dim)]">{m.email}</span>
                      </Td>
                      <Td>
                        <RoleChip role={m.role} lang={lang} />
                      </Td>
                      <Td>
                        <span className="text-[var(--text-dim)] num">
                          {fmtDate(m.joinedAt)}
                        </span>
                      </Td>
                      {canManage && (
                    ...
                    {rowActive && action.kind === "changeRole" && (
                      <Tr>
                        <Td colSpan={colSpan} className="bg-[var(--surface)]">
                          <div className="flex flex-col md:flex-row md:items-center gap-3">
                            ...
                            <select
                              ...
                              className="h-9 rounded-lg border border-[var(--border)] bg-[var(--card)] text-[var(--text)] px-3 text-[13px]"
                            >
                              <option value="ADMIN">
      ...
      {error && (
        <div className="px-5 py-3 text-[12px] text-[var(--red)] border-t border-[var(--border)]">
          {error}
        </div>
      )}
    </Card>
