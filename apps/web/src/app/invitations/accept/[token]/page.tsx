...
export default async function AcceptInvitationPage({ params }: Props) {
  ...
  return (
    <div className="min-h-screen grid place-items-center px-4 bg-[var(--bg)]" dir={dir}>
      <div className="w-full max-w-md">
        ...
        <Card>
          <CardHeader>
            <h1 className="text-xl font-semibold text-[var(--text)]">{c.title}</h1>
          </CardHeader>
          <CardBody>
            {status !== "ok" && (
            ...
            {status === "ok" && invitation && (
              <>
                ...
                {!user && (
                  <>
                    <p className="text-[13px] text-[var(--text-mid)] mb-3">
